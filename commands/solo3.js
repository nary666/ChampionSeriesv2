...
module.exports.run = async (blaki, message, args) => {
    ...
    const gra5 = "**KLUCZ DO GRY 4 POJAWI SIĘ O GODZINIE 18:30**"
    const check = '609329690318077962'
  
    if(!message.member.roles.has(HOST.id)) return message.reply("Ooops, nie posiadasz uprawnień!");
    if(!args[0]) return message.channel.send("❌ _Wprowadź prawidłowe wartości, **.solo3 hasło **_ ❌").then(() =>
    {
    ...
    .setColor("#FFC125")
    .setTitle("**ARENA SOLO CUSTOM - GAME 3 !**")
